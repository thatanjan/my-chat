...
export default function ChatList({ onSelectChat }: ChatListProps) {
  ...
  return (
    <div className={`border-r border-gray-700 flex flex-col transition-all duration-300 ${isSidebarOpen ? 'w-80' : 'w-20'}`}>
      <div className="p-4 border-b border-gray-700 flex items-center justify-between">
        <Button variant="ghost" size="icon" onClick={() => setIsSidebarOpen(!isSidebarOpen)}>
          <Menu className="h-6 w-6" />
        </Button>
        {isSidebarOpen && (
          <Input placeholder="Search chats..." className="bg-gray-800 border-gray-700 text-white ml-2" />
        )}
      </div>
      <ScrollArea className="flex-grow">
        {isSidebarOpen ? (
          ...
          <div className="flex flex-col items-center space-y-4 py-4">
            {chats.map((chat) => (
              <div key={chat.id} className="relative" onClick={() => onSelectChat(chat.id)}>
